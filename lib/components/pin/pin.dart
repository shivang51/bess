...
class Pin extends Component {
  ...
  void simulate(BuildContext context, DigitalState state, String callerId) {
    ...
    drawAreaData.updateComponentProperty(
        properties.id, ComponentPropertyType.digitalState, state);

    for (var connectedPinId in properties.connectedPinsIds) {
      if (connectedPinId == callerId) continue;

      var cProperties = (drawAreaData.components[connectedPinId] as Pin)
          .properties as PinProperties;
    ...
    if (properties.parentId != callerId) {
      drawAreaData.components[properties.parentId]
          ?.simulate(context, state, properties.id);
    }
  }
  ...
  void remove(BuildContext context) {
    ...
    for (var wire in properties.connectedWiresIds.entries) {
      ...
      var connectedPinProperties =
          (drawAreaData.components[pinId]!.properties as PinProperties);
