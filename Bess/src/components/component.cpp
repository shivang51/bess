...
namespace Bess::Simulator::Components {
	Component::Component(const UUIDv4::UUID& uid, int renderId, glm::vec2 position, ComponentType type) : m_uid(uid), m_renderId(renderId), m_position(position), m_type(type) {}

	int Component::getRenderId() const {
		return m_renderId;
	}

	UUIDv4::UUID Component::getId() const {
		return m_uid;
	}
	...
	void Component::onEvent(ComponentEventData e) {
		if (m_events.find(e.type) == m_events.end()) return;
