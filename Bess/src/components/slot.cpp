...
namespace Bess::Simulator::Components {
    ...
    void Slot::deleteComponent()
    {
        ...
        uuids::uuid iId, oId;
        if (m_type == ComponentType::inputSlot) iId = m_uid;
        else oId = m_uid;

        for (const auto& connSlot : m_connections) {
            if (m_type == ComponentType::inputSlot) oId = connSlot;
            else iId = connSlot;
    ...
    void Slot::onLeftClick(const glm::vec2& pos) {
        ...
        if (slot == nullptr || ApplicationState::connStartId == m_uid || slot->getType() == m_type) goto clear;
    ...
    void Slot::addConnection(const uuids::uuid& uid, bool simulate) {
        ...
        if (!simulate || m_type == ComponentType::inputSlot) return;
    ...
    void Slot::removeConnection(const uuids::uuid& uid)
    {
        ...
        int i = 0;
        while (i < m_connections.size() && m_connections[i] != uid) i++;
        if (i == m_connections.size()) return;
        m_connections.erase(m_connections.begin() + i);
        if (m_type == ComponentType::inputSlot && m_stateChangeHistory.find(uid) != m_stateChangeHistory.end()) m_stateChangeHistory.erase(uid);
    }
} // namespace Bess::Simulator::Components