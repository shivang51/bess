...
namespace Bess::Gl {
    ...
    GLuint Shader::createProgram(const std::string &vertexPath,
        ...
        const auto vertexShader = readFile(vertexPath);
        const auto fragmentShader = readFile(fragmentPath);
        const auto tessalationShader = readFile(tessalationPath);
        const auto evaluationShader = readFile(evaluationPath);

        const auto vertexShaderId = compileShader(vertexShader.c_str(), GL_VERTEX_SHADER);
        const auto fragmentShaderId =
            compileShader(fragmentShader.c_str(), GL_FRAGMENT_SHADER);
        const auto tessalationShaderId =
            compileShader(tessalationShader.c_str(), GL_TESS_CONTROL_SHADER);
        const auto evaluationShaderId =
    ...
    GLuint Shader::createProgram(const std::string &vertexPath,
        ...
        const auto vertexShaderId = compileShader(vertexShader.c_str(), GL_VERTEX_SHADER);
        const auto fragmentShaderId = compileShader(fragmentShader.c_str(), GL_FRAGMENT_SHADER);
    ...
    GLuint Shader::compileShader(const std::string &shaderSrc, const GLenum shaderType) {
    ...
    void Shader::setUniform1iv(const std::string &name, const int *values, const int count) {
        if (!m_uniformLocationCache.contains(name)) {
            m_uniformLocationCache[name] = glGetUniformLocation(m_id, name.c_str());
        }
