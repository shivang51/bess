...
namespace Bess::Canvas {
    ...
    void NodesArtist::drawSimEntity(entt::entity entity) {
    ...
    void NodesArtist::drawSimEntity(
        ...
        // Handle headerless components (INPUT, OUTPUT, STATE_MONITOR) in nodes view
        if (tagComp.type.simCompType == SimEngine::ComponentType::INPUT ||
            tagComp.type.simCompType == SimEngine::ComponentType::OUTPUT ||
            tagComp.type.simCompType == SimEngine::ComponentType::STATE_MONITOR) {
        ...
        scale.y = 18.f + 4.f + (maxRows * SLOT_ROW_SIZE); // headerHeight + rowGap + (maxRows * SLOT_ROW_SIZE)
        transform.scale = scale;

        float headerHeight = 18.f; // componentStyles.headerHeight
        ...
        glm::vec3 textPos = glm::vec3(pos.x - scale.x / 2.f + 8.f, headerPos.y + 4.f, pos.z + 0.0005f); // paddingX, paddingY
        ...
        Renderer::msdfText(tagComp.name, textPos, 10.f, ViewportTheme::colors.text, id, rotation); // headerFontSize
    ...
    void NodesArtist::drawHeaderLessComp(entt::entity entity,
        ...
        auto labelSize = Renderer::getMSDFTextRenderSize(tagComp.name, 10.f); // headerFontSize

        uint64_t id = (uint64_t)entity;
        auto pos = transform.position;
        auto rotation = transform.angle;
        auto scale = transform.scale;
        float columnSize = SLOT_COLUMN_SIZE;

        float xSize = labelSize.x;
        xSize += 8.f * 2.f; // paddingX * 2

        float labelXGapL = 0.f, labelXGapR = 0.f;
        float labelLOffset = 8.f; // paddingX
        ...
        glm::vec3 textPos = glm::vec3(
            ...
            pos.y + (10.f / 2.f) - 1.f, pos.z + 0.0005f); // headerFontSize / 2

        auto name = tagComp.name;
        Renderer::msdfText(name, textPos, 10.f, ViewportTheme::colors.text, id); // headerFontSize
    ...
    glm::vec3 NodesArtist::getSlotPos(const Components::SlotComponent &comp, const Components::TransformComponent &parentTransform) {
        ...
        const auto &isNonHeader = m_sceneRef->getEnttRegistry().any_of<Components::SimulationInputComponent, Components::SimulationOutputComponent, Components::SimulationStateMonitor>(parentEntt);
        if (!isNonHeader)
            posY += SLOT_START_Y;
