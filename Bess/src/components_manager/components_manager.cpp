...
namespace Bess::Simulator {
	...
	std::unordered_map<UUIDv4::UUID, ComponentsManager::ComponentPtr> ComponentsManager::components;

	std::unordered_map<UUIDv4::UUID,  ComponentsManager::ComponentPtr> ComponentsManager::renderComponenets;
	...
	void ComponentsManager::generateNandGate(const glm::vec2& pos) {
		// input slots
		int n = 2;
		...
		while (n--) {
			...
			components[uid] = std::make_shared<Components::Slot>(uid, renderId, ComponentType::inputSlot);
		...
		while (n--) {
			auto uid = uuidGenerator.getUUID();
			auto renderId = getRenderId();
			components[uid] = std::make_shared<Components::Slot>(uid, renderId, ComponentType::outputSlot);
			renderIdToCId[renderId] = uid;
			compIdToRId[uid] = renderId;
		...
		auto uid = uuidGenerator.getUUID();
		auto renderId = getRenderId();
		components[uid] = std::make_shared<Components::NandGate>(uid, renderId, pos, inputSlots, outputSlots);
		renderIdToCId[renderId] = uid;
		compIdToRId[uid] = renderId;
	...
	void ComponentsManager::generateInputProbe(const glm::vec2& pos) {
		auto slotId = uuidGenerator.getUUID();
		auto renderId = getRenderId();
		components[slotId] = std::make_shared<Components::Slot>(slotId, renderId, ComponentType::outputSlot);
		renderIdToCId[renderId] = slotId;
		compIdToRId[slotId] = renderId;

		auto uid = uuidGenerator.getUUID();
		renderId = getRenderId();
		components[uid] = std::make_shared<Components::InputProbe>(uid, renderId, pos, slotId);
		renderIdToCId[renderId] = uid;
		compIdToRId[uid] = renderId;
		renderComponenets[uid] = components[uid];
	}

    void ComponentsManager::generateOutputProbe(const glm::vec2& pos) {
        auto slotId = uuidGenerator.getUUID();
        auto renderId = getRenderId();
        components[slotId] = std::make_shared<Components::Slot>(slotId, renderId, ComponentType::inputSlot);
        renderIdToCId[renderId] = slotId;
        compIdToRId[slotId] = renderId;

        auto uid = uuidGenerator.getUUID();
        renderId = getRenderId();
        components[uid] = std::make_shared<Components::OutputProbe>(uid, renderId, pos, slotId);
        renderIdToCId[renderId] = uid;
        compIdToRId[uid] = renderId;
        renderComponenets[uid] = components[uid];
    }

	void ComponentsManager::generateConnection(const UUIDv4::UUID& slot1, const UUIDv4::UUID& slot2) {
        ...
        auto uid = uuidGenerator.getUUID();
        auto renderId = getRenderId();
        components[uid] = std::make_shared<Components::Connection>(uid, renderId, slotA->getId(), slot2);
        renderIdToCId[renderId] = uid;
        compIdToRId[uid] = renderId;
        renderComponenets[uid] = components[uid];
	}
	...
	//private
	int ComponentsManager::getRenderId() {
