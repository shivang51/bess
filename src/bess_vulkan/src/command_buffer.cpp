...
namespace Bess::Vulkan {
    VulkanCommandBuffer::VulkanCommandBuffer(VkCommandBuffer vkHandle)
        : m_vkCmdBufferHandel(vkHandle) {}
    ...
    VkCommandBuffer VulkanCommandBuffer::beginRecording() const {
        ...
        if (vkBeginCommandBuffer(m_vkCmdBufferHandel, &beginInfo) !=
            VK_SUCCESS) {
    ...
    VulkanCommandBuffers::VulkanCommandBuffers(
        const std::shared_ptr<VulkanDevice> &device, size_t count)
        : m_device(device) {
        ...
        for (size_t i = 0; i < count; i++) {
            m_commandBuffers.emplace_back(
                std::move(std::make_shared<VulkanCommandBuffer>(
                    m_commandBuffersVkHnd[i])));
        }
    }

    VulkanCommandBuffers::~VulkanCommandBuffers() {
        vkFreeCommandBuffers(m_device->device(), m_commandPool,
    ...
    VulkanCommandBuffers::getCmdBuffers() const {
