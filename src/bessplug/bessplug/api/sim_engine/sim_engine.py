...
def expr_sim_function(inputs: list[PinState], simTime: float, oldState: NativeComponentState) -> 'ComponentState':
    """A simulation function that evaluates expressions."""
    return expr_eval_sim_func([input._native for input in inputs], datetime.timedelta(microseconds=simTime / 1000), oldState)
