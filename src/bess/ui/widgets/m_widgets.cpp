...
namespace Bess::UI::Widgets {
    ...
    bool CheckboxWithLabel(const char *label,
        ...
        if (expandToFullWidth) {
            const float checkboxWidth =
                ImGui::CalcTextSize("X").x + style.FramePadding.x;
            ...
            ImGui::SameLine();
            ImGui::SetCursorPosX(textWidth + style.ItemInnerSpacing.x +
    ...
    bool TreeNode(int key,
        ...
        const ImGuiID id =
            window->GetID(std::format("@{}@_{}", key, name).c_str());
        ImGui::PushID(id);
        ...
        if (!icon.empty()) {
            if (iconColor.x >= 0.0f) {
                ImU32 col = ImGui::GetColorU32(
                    ImVec4(iconColor.x, iconColor.y, iconColor.z, iconColor.w));
                ImGui::PushStyleColor(ImGuiCol_Text, col);
            }

            ImGui::RenderText(
                ImVec2(pos.x + buttonSize + (g.Style.ItemInnerSpacing.x * 2),
                       pos.y + g.Style.FramePadding.y),
                icon.c_str());

            posIconOffsetX = ImGui::CalcTextSize(icon.c_str()).x +
        ...
        const float posX =
            pos.x + buttonSize + posIconOffsetX + g.Style.ItemInnerSpacing.x;
    ...
    bool ButtonWithPopup(const std::string &label,
        ...
        const float menuBtnSizeX = showMenuButton ? menuBtnSizeY : 0;
        const float menuBtnX =
            showMenuButton
        ...
        const ImRect bbMenuButton(
            ...
            ImVec2(pos.x + menuBtnX + menuBtnSizeX, pos.y + menuBtnSizeY));

        const ImGuiID id = window->GetID(label.c_str());

        bool hovered = false, held = false;
        const bool clicked = ImGui::ButtonBehavior(
            bbButton, id, &hovered, &held, ImGuiButtonFlags_PressedOnClick);
        ...
        if (menuHovered || hovered || held ||
            ImGui::IsPopupOpen(popupName.c_str()))
            bgColor = ImGui::GetColorU32(held ? ImGuiCol_ButtonActive
                                              : ImGuiCol_ButtonHovered);

        const auto rounding = style.FrameRounding;
        window->DrawList->AddRectFilled(bb.Min, bb.Max, bgColor, rounding);
        ImGui::RenderText(
            ImVec2(pos.x + style.FramePadding.x + g.Style.ItemInnerSpacing.x,
                   pos.y + g.Style.FramePadding.y),
            label.c_str());
        ...
        if (showMenuButton && (hovered || menuHovered)) {
            ...
            const float x = bbMenuButton.Min.x +
                            ...
                            3.f;
            ImGui::RenderText(ImVec2(x, pos.y + g.Style.FramePadding.y),
                              Icons::FontAwesomeIcons::FA_ELLIPSIS_VERTICAL);
            if (menuClicked)
    ...
    std::pair<bool, bool> EditableTreeNode(uint64_t key,
        ...
        const ImVec2 pos = {window->DC.CursorPos.x - 2.f,
        ...
        bool opened =
            st->GetInt(openId,
                       (treeFlags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) !=
            0;
        ...
        const auto chevronIcon =
            opened ? Icons::FontAwesomeIcons::FA_CHEVRON_DOWN
                   : Icons::FontAwesomeIcons::FA_CHEVRON_RIGHT;
        const auto stateIcon =
            std::format(" {}  {}", chevronIcon, opened ? "" : " ");

        const float toggleWidth = ImGui::CalcTextSize(stateIcon.c_str()).x +
                                  g.Style.ItemInnerSpacing.x;
        const ImRect toggleRect(rowBB.Min,
        ...
        // NOTE: Drawing button advances cursor, so we must restore it later to
        // avoid layout glitches (shrinking height).
        ImVec2 backupCursorPos = window->DC.CursorPos;
        if (rowHovered) {
            ImGui::SetCursorScreenPos(
                ImVec2(rowBB.Max.x - g.FontSize - g.Style.FramePadding.x,
                       rowBB.Min.y + g.Style.FramePadding.y));
        ...
        ImVec2 labelPos(toggleRect.Max.x + iconOffsetX +
                            g.Style.ItemInnerSpacing.x,
                        rowBB.Min.y + g.Style.FramePadding.y);
        ImVec2 labelSize = ImGui::CalcTextSize(name.c_str());
        ImRect labelRect(labelPos,
                         ImVec2(labelPos.x + labelSize.x, rowBB.Max.y));
        ...
        if (editing) {
            ...
            if (!editBuf) {
                ...
                if (ImGui::IsKeyPressed(ImGuiKey_Enter))
                    commit = true;
                // this is true when the item was active and then deactivated
                // after edit (e.g., user clicked away)
                ...
                if (commit || cancel) {
                    if (cancel) {
                        std::string *prevName =
                            (std::string *)st->GetVoidPtr(prevNameId);
                        if (prevName) {
                            name = *prevName;
                        }
                        delete prevName;
                    ...
                    std::string *editBufPtr =
                        (std::string *)st->GetVoidPtr(editBufId);
                    delete editBufPtr;
        ...
        // Restore cursor again just to be safe after InputText/RenderText which
        // might move it
        window->DC.CursorPos = backupCursorPos;
