...
namespace Bess::Vulkan::Pipelines {
    ...
    PathStrokePipeline::~PathStrokePipeline() { cleanup(); }
    ...
    void PathStrokePipeline::endPipeline() {
        ...
        VkDeviceSize bufferSize =
            m_strokeVertices.size() * sizeof(CommonVertex);
        vkMapMemory(m_device->device(),
                    m_vertexBufferMemory[m_currentFrameIndex], vertexOffset,
                    bufferSize, 0, &data);
        ...
        vkMapMemory(m_device->device(),
                    m_indexBufferMemory[m_currentFrameIndex], indexOffset,
                    bufferSize, 0, &data);
        memcpy(data, m_strokeIndices.data(), bufferSize);
        vkUnmapMemory(m_device->device(),
                      m_indexBufferMemory[m_currentFrameIndex]);

        vkCmdBindVertexBuffers(m_currentCommandBuffer, 0, 1, &vertexBuffer,
                               &vertexOffset);
        vkCmdBindIndexBuffer(m_currentCommandBuffer,
                             m_indexBuffers[m_currentFrameIndex], indexOffset,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(m_currentCommandBuffer,
                         static_cast<uint32_t>(m_strokeIndices.size()), 1, 0, 0,
                         0);
    ...
    void PathStrokePipeline::setPathData(
        const std::vector<CommonVertex> &strokeVertices,
    ...
    void
    PathStrokePipeline::updateUniformBuffer(const UniformBufferObject &ubo) {
    ...
    void PathStrokePipeline::ensurePathCapacity(size_t vertexCount,
                                                size_t indexCount) {
