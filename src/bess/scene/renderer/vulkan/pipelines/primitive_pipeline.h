...
namespace Bess::Vulkan::Pipelines {
    ...
    class PrimitivePipeline : public Pipeline {
      ...
      private:
        ...
        void setTextureSetGrowthPolicy(float growthFactor, uint32_t minHeadroom,
                                       uint32_t maxSetsCap);
        size_t textureSetsPerFrame() const;
        size_t textureSetFrameBase() const;
        size_t textureSetFrameLimit() const;

        VkDescriptorPool createTextureDescriptorPool(uint32_t maxSets,
                                                     uint32_t descriptorCount);
        void createTextureDescriptorSets(uint32_t descCount, uint32_t setsCount,
                                         VkDescriptorPool pool,
                                         VkDescriptorSetLayout &layout,
                                         std::vector<VkDescriptorSet> &sets);
        void bindDescriptorSets(bool isTranslucent,
                                VkDescriptorSet textureSet) const;
        void updateTextureSet(size_t descriptorSetIndex,
                              const std::array<VkDescriptorImageInfo,
                                               m_texArraySize> &textureInfos);
        ...
        struct TempDescSets {
            ...
            VkDescriptorSet getSetAtIdx(uint64_t idx) {
                if (!isDescSetAvailable(idx)) {
                    BESS_WARN("[PrimitivePipeline] Descriptor set not found at "
                              "idx {} for poolSize = {}, setsCount = {}",
                              idx, pools.size(),
                              pools.size() * DESC_SET_COUNT_PER_POOL);
            ...
            std::pair<VkDescriptorPool &, std::vector<VkDescriptorSet> &>
            reserveNextPool() {
        ...
        std::unordered_map<size_t,
                           std::array<VkDescriptorImageInfo, m_texArraySize>>
            m_cachedTextureInfos;
