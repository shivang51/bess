...
> Generated by LLM (I audited some of it)
...
Use `getYogaNode()` only for diagnostics, tests, or Yoga features that are not
yet wrapped by `UINode`.

```cpp
YGNodeRef yogaNode = node->getYogaNode();
```

Prefer adding a typed `UINode` wrapper method when the feature becomes part of
normal UI code.
...
- Confirm the node was added to `UINodeRegistry`.
- Confirm parent-child relationships are created with `addChild()`.
- Confirm layout is run from a root node after UI preparation.
- For right/center alignment, confirm the aligning container has a definite width
  or is stretched by its parent.
- Avoid percentage sizes inside wrap-content parents unless the parent has a
  definite size.
- Use `getDrawSize()` and `getDrawPos()` for rendering, not transform scale.
