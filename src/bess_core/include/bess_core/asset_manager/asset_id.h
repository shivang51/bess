...
namespace Bess::Assets {
    ...
    constexpr uint64_t fnv1a_hash_step(uint64_t current_hash,
    ...
    template <typename TAsset, size_t N> class AssetID {
      public:
        const uint64_t id;
        ...
        constexpr explicit AssetID(TPaths... assetPaths)
            : id(fnv1a_hash_combined(assetPaths...)),
              paths{assetPaths...} {
            static_assert(sizeof...(assetPaths) == N,
                          "Internal error: Number of paths does not match "
                          "template argument N");
        }
