...
// Themes are mostly generated by LLMs
...
namespace Bess::Config {
    ...
    void Themes::applyTheme(const std::string &themeName) {
        ...
        if (theme == nullptr) {
            theme = isDark ? Core::Style::BessTheme::defaultDarkTheme()
        ...
        GAppContext::getInstance()
            ...
            ->queue(Bess::Events::ThemeChangeEvent{
                isDark,
                theme,
            });
    }
    ...
    std::shared_ptr<Core::Style::BessTheme> &Themes::getCurrentThemeRef() {
        static std::shared_ptr<Core::Style::BessTheme> currentTheme = nullptr;
        return currentTheme;
    }

    std::shared_ptr<Core::Style::BessTheme> Themes::getCurrentTheme() {
        return getCurrentThemeRef();
    }
    ...
    void Themes::setBessMinimalColors() {
        ...
        // Blue accent used for selections, checkmarks, timeline markers,
        // etc.
    ...
    void Themes::setFluentUIColors() {
        ...
        colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.50f,
                   0.50f,
                   0.50f,
                   1.00f); // Slightly lighter gray when active
    ...
    namespace {
        void setImGuiColors(const Bess::Core::Style::ColorScheme &colorScheme,
            ...
            // Following mappings are generated by LLM
            ...
            colors[ImGuiCol_SeparatorHovered] = getImVec4(
                themeColors.outline); // Accentuate line, don't colorize wildly
            colors[ImGuiCol_SeparatorActive] = getImVec4(themeColors.primary);

            // Text
            colors[ImGuiCol_Text] = getImVec4(themeColors.onSurface);
            colors[ImGuiCol_TextDisabled] =
                withAlpha(themeColors.onSurface, 0.38f);

            // Headers / tree nodes / collapsing bars
            // Shift cleanly within the container hierarchy for consistent
            // text contrast
            colors[ImGuiCol_Header] =
                getImVec4(themeColors.surfaceContainerLow);
            colors[ImGuiCol_HeaderHovered] =
            ...
            colors[ImGuiCol_FrameBg] =
                getImVec4(themeColors.surfaceContainerHighest);
            colors[ImGuiCol_FrameBgHovered] =
                withAlpha(themeColors.primary,
                          isDark ? 0.08f : 0.15f); // Micro-tinting on hover
            colors[ImGuiCol_FrameBgActive] =
                withAlpha(themeColors.primary, isDark ? 0.16f : 0.24f);

            // Buttons
            // Safe containers that cleanly match global ImGuiCol_Text
            colors[ImGuiCol_Button] =
                getImVec4(themeColors.surfaceContainerHigh);
            colors[ImGuiCol_ButtonHovered] =
                getImVec4(themeColors.surfaceContainerHighest);
            colors[ImGuiCol_ButtonActive] = getImVec4(
                themeColors.outlineVariant); // Muted, dynamic alternate layer
            ...
            colors[ImGuiCol_SliderGrabActive] = getImVec4(themeColors.primary);

            // Tabs
            // Fixed contrast stacking so unfocused tabs don't melt away
            colors[ImGuiCol_Tab] = getImVec4(themeColors.surfaceContainerLow);
            colors[ImGuiCol_TabHovered] =
                getImVec4(themeColors.surfaceContainerHighest);
            ...
            // Resize grips
            colors[ImGuiCol_ResizeGrip] =
                withAlpha(themeColors.outlineVariant, 0.40f);
            colors[ImGuiCol_ResizeGripHovered] =
            ...
            colors[ImGuiCol_PlotLinesHovered] =
                getImVec4(themeColors.error); // Used error color to catch
                                              // attention on mouseover
            colors[ImGuiCol_PlotHistogram] = getImVec4(themeColors.secondary);
            colors[ImGuiCol_PlotHistogramHovered] =
                getImVec4(themeColors.primary);
            ...
            colors[ImGuiCol_TableRowBgAlt] =
                withAlpha(themeColors.onSurface, 0.03f); // Tiny shade variation

            // Selection & drag-drop
            colors[ImGuiCol_TextSelectedBg] =
                withAlpha(themeColors.primary, 0.25f);
            ...
            colors[ImGuiCol_ModalWindowDimBg] =
                withAlpha(themeColors.scrim, 0.45f);
        }
    } // namespace
